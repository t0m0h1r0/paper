...
% 証明終了記号は amsthm デフォルトの □（hollow square）を使用
...
合成半マルコフ過程として定式化し，
コンテナベースの復旧モデル（隔離・クローン・再デプロイの三段階）を含む定常コストレートを最小化するバックアップ設計問題を扱う．
...
$\Dinf\approx19.7$ 日（約 3 週間）となり，実際の IT 運用慣行と整合する．
また動的保持延長を，ストレージコストと残存リスクをバランスする
単期間費用最適化問題として定式化し，閉形式整数最適解を与える（命題~\ref{prop:newsboy}）．
\end{abstract}
...
~\cite{mandiant2023,ibm2023,verizon2023,sophos2023,crowdstrike2023}，
...
\item \textbf{最適バックアップ間隔の閉形式}：
  ...
  $\Dinf\approx19.7$ 日となり，実運用の週次〜月次バックアップと整合する．
...
第~\ref{sec:newsboy} 節で確率的保持延長最適化問題を扱う．
...
\begin{tikzpicture}[
  node distance=2.2cm,
  state/.style={circle, draw, minimum size=1.1cm, font=\small},
  ...
  \node[state, right of=SS, fill=gray!20] (SF) {$S_F$};

  \draw (SN) -- node[above]{\footnotesize $\lambda$} (SL);
  \draw (SL) -- node[above left]{\footnotesize $\beta(I)$} (SE);
  \draw (SL) -- node[below left]{\footnotesize $T_d$後} (SS);
  \draw (SE) -- node[above]{\footnotesize 隔離} (SI);
  \draw (SS) -- node[above]{\footnotesize $\eta$} (SI);
  \draw (SS) -- node[below]{\footnotesize $1-\eta$} (SF);
  \draw (SI) -- node[above]{\footnotesize 復旧} (SR);
  \draw[bend right=40] (SR) to node[below]{\footnotesize 完了} (SN);
\end{tikzpicture}
\caption{半マルコフ状態遷移図．
$S_N$：正常，$S_L$：潜伏感染中（未検知），$S_E$：早期検知済，
$S_S$：発症後・症状検知待ち，$S_I$：隔離済，$S_R$：復旧中，$S_F$：全損（灰色）．
$\lambda$：攻撃到着レート，$\beta(I)$：早期検知レート，$\eta$：症状検知確率．}
...
\begin{definition}[コンテナベース復旧時間モデル]\label{def:recovery}
現代のクラウドネイティブ・インフラでは，サービスは Docker 等のコンテナとして
パッケージされ，Kubernetes 等のオーケストレーション基盤上で稼働する~\cite{nist80034,cisa2020}．
ランサムウェア感染が検知された場合，復旧手順は以下の三段階から成る：
\begin{enumerate}[leftmargin=2em,label=(\roman*)]
  \item \emph{隔離}（$T_{\mathrm{iso}}\sim\mathrm{Exp}(\mu_{\mathrm{iso}})$）：
    感染コンテナをネットワークから切り離す．
  \item \emph{クローン}（$T_{\mathrm{clone}}\sim\mathrm{Exp}(\mu_c)$）：
    汚染されていない直近バックアップ世代から，
    クリーンなコンテナイメージを複製する．
  \item \emph{再デプロイ}（$T_{\mathrm{deploy}}\sim\mathrm{Exp}(\mu_d)$）：
    複製したイメージをサービス基盤に再展開し，動作確認を行う．
\end{enumerate}
全停止時間は $\Tdown=T_{\mathrm{iso}}+\Trec$，
$\Trec=T_{\mathrm{scan}}+T_{\mathrm{clone}}+T_{\mathrm{deploy}}$（$T_{\mathrm{scan}}=\kappa_s\Tdet$，$\kappa_s>0$：フォレンジックスキャン係数）．
平均復旧時間を $\tau_R:=\mu_{\mathrm{iso}}^{-1}+\mu_c^{-1}+\mu_d^{-1}$ と書く．

各フェーズに指数分布を仮定するのは解析上の便宜であり，
実環境での決定論的・位相型分布への拡張は今後の課題である．
\end{definition}

\subsection{完全モデルの期待損失と費用レート}

更新サイクルあたりの期待損失 $\Qf$ を次のように定義する：
...
$\Dinf$ が設計の下限として機能する（第~\ref{sec:numerical} 節で $\lambda\ge0.1$ のとき誤差 $<0.1$ 日を確認）．
\end{corollary}
...
（単期間費用最適化として命題~\ref{prop:newsboy} で正確に扱う）．
\end{proposition}

%%==========================================================
\section{動的保持延長：単期間費用最適化問題}
\label{sec:newsboy}
%%==========================================================

\begin{tcolorbox}[mathbox, title={単期間費用最適化問題の構造}]
本節の問題は「追加保持コスト（確実に発生）」と
「保持不足による復旧不能ペナルティ（確率的に発生）」の二種類の費用を
同時に最小化する問題である~\cite{porteus2002,zipkin2000}．
追加世代数を増やすほど保持コストは単調増加するが，
復旧不能確率（Pareto 裾）は単調減少する．
この凸性が唯一の最適解の存在を保証し，閉形式の解を導く~\cite{silver1998}．
\end{tcolorbox}
...
トレードオフを最小化する単期間費用最適化問題の一例である~\cite{porteus2002}．
重尾（$\alpha$小）ほどペナルティ項の減衰が遅く，最適な $n_{\mathrm{ext}}^*$ が大きくなる．

\subsection{閉形式の整数最適解}

\begin{proposition}[最適追加保持世代数]\label{prop:newsboy}
...
\begin{table}[H]
\centering\small
\caption{ベースラインパラメータと出典（社会インフラ・エンタープライズ規模想定）}
\label{tab:params}
\begin{tabular}{@{}lll p{72mm}@{}}
\toprule
記号 & 値 & 単位 & 根拠・出典 \\
\midrule
$t_m$ & 1 & 日 & ランサムウェア潜伏の観測最小値~\cite{mandiant2023,crowdstrike2023} \\
$\lambda$ & 1.0 & 件/日 & 金融・電力等の重要インフラを対象とした
  国家支援型 APT 攻撃の推定到達頻度：
  NISC の重要インフラ防護報告~\cite{nisc2023} および
  JPCERT/CC~\cite{jpcert2023} の大規模組織向け分析に基づく \\
$d_1$ & 1667 & 万円/日 & $d_1=L/\mathrm{MTPD}$ の定義による（MTPD=30 日） \\
$L$ & 50000 & 万円 & 社会インフラ事業者におけるランサムウェア被害の
  推定損失規模（復旧費用・機会損失・信用毀損の合算）：
  IPA~\cite{ipa2024}・警察庁~\cite{npa2023} の大規模組織被害実態調査，
  および海外事例（Colonial Pipeline, Change Healthcare 等）の換算値~\cite{ibm2023} \\
$c_b$ & 10 & 万円$\cdot$日 & エンタープライズ向けバックアップ基盤の運用固定費 \\
$n$ & 5 & 世代 & CISA の推奨する "3-2-1-1-0" バックアップ
  原則~\cite{cisa2020} に基づく最小世代数 \\
$\tau_R$ & 2.0 & 日 & コンテナ隔離（$\approx0.5$ 日）+ クローン（$\approx1.0$ 日）+
  再デプロイ（$\approx0.5$ 日）の合計：
  NIST SP 800-184~\cite{nist80034} および CISA~\cite{cisa2020} の
  大規模システム向け復旧時間指標の中央値 \\
$\eta$ & 0.95 & --- & 主要 EDR ソリューション（CrowdStrike Falcon 等）の
  企業環境における症状検知率~\cite{crowdstrike2023,sophos2023} \\
$\kappa_s$ & 0.3 & 日/世代 & エンタープライズ向けフォレンジックスキャン所要時間：
  CISA~\cite{cisa2020} のインシデント対応ガイドライン概算 \\
$\beta_{\max}$ & 0.5 & 1/日 & 高度 EDR 導入時の事前検知レート上限~\cite{crowdstrike2023} \\
$c_n$ & 50 & 万円/世代 & エンタープライズ規模（$\approx5\,\mathrm{TB}$/世代）の
  クラウドストレージ月額費用相当 \\
\bottomrule
\end{tabular}
\end{table}
...
理論値の確認：
$\La=50000-0.95\times(1666.7\times2.0+0)=46833$ 万円，
$\Dinf=46833/(0.95\times1666.7\times0.3\times5)=46833/2375=19.72$ 日（約3週間）．
...
\subsection{定理~\ref{thm:dQ} の数値確認：$\Dinf=19.72$ 日での唯一の零点}
...
$\partial\Qf/\partial\Delta$ & $\partial^2\Qf/\partial\Delta^2$ \\
\midrule
10    & $-146.9$ & $+37.15$ & $-72.7$ & $+20.56$ & $-8.81$ & $+3.110$ \\
19.72 & $0.00$   & $+5.458$ & $0.00$  & $+2.203$ & $0.00$  & $+0.166$ \\
25    & $+20.19$ & $+2.612$ & $+7.59$ & $+0.891$ & $+0.48$ & $+0.043$ \\
30    & $+29.90$ & $+1.414$ & $+10.64$ & $+0.397$ & $+0.60$ & $+0.008$ \\
\bottomrule
\end{tabular}
\end{table}

全 $\alpha$ において，$\partial^2\Qf/\partial\Delta^2>0$（厳密凸）かつ $\partial\Qf/\partial\Delta$ が
$\Dinf=19.72$ 日で唯一の零点を持つことが確認される．

\subsection{定理~\ref{thm:convergence} の数値確認：$\lambda$ 非依存 $M_0$ と収束}

\begin{table}[H]
\centering
\caption{$\lambda$ 非依存上界 $M_0$ の検証（$\lambda_0=0.1$，$\alpha=0.7$）：
$\Qf(\Dopt(\lambda))\le M_0=7506.59$ が全 $\lambda\ge\lambda_0$ で成立}
\label{tab:compact}
\begin{tabular}{@{}lrrrl@{}}
\toprule
$\lambda$ & $\Dopt(\lambda)$（日）& $\Qf(\Dopt)$ & $M_0=7506.59$ & 判定 \\
\midrule
0.1  & 19.80 & 7501.53 & 7506.59 & $\le\checkmark$ \\
1.0  & 19.73 & 7501.52 & 7506.59 & $\le\checkmark$ \\
10.0 & 19.72 & 7501.52 & 7506.59 & $\le\checkmark$ \\
100  & 19.72 & 7501.52 & 7506.59 & $\le\checkmark$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{$\Dopt(\lambda)\to\Dinf=19.72$ 日の収束（全 $\alpha$）}
\label{tab:convergence}
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
$\lambda$ & 0.01 & 0.1 & 1.0 & 10 & 100 & $\Dinf$ \\
\midrule
$\alpha=0.5$ & 20.19 & 19.77 & 19.72 & 19.720 & 19.719 & \textbf{19.72} \\
$\alpha=0.7$ & 20.55 & 19.80 & 19.73 & 19.720 & 19.719 & \textbf{19.72} \\
$\alpha=1.0$ & 22.06 & 19.95 & 19.74 & 19.722 & 19.720 & \textbf{19.72} \\
$\alpha=1.5$ & 42.44 & 21.33 & 19.88 & 19.735 & 19.721 & \textbf{19.72} \\
\bottomrule
\end{tabular}
\end{table}

$\alpha=1.5$（命題~\ref{prop:gap} のギャップが小さい）では $\lambda=0.01$ での偏差が最大（42.4 日）だが，
$\lambda\ge0.1$（現実的な攻撃頻度域）では全 $\alpha$ で $\Dinf$ と $1.7$ 日以内に収束する．
...
$Q''_\infty=3.051$，補正係数 $0.270/\lambda$）}
\label{tab:taylor}
\begin{tabular}{@{}lrrrr@{}}
\toprule
$\lambda$ & $\Dopt$（数値）& $\Dopt$（Taylor 近似）& 絶対誤差（日）\\
\midrule
0.10 & 19.803 & 19.804 & $<0.001$ \\
1.00 & 19.728 & 19.728 & $<0.001$ \\
10.0 & 19.720 & 19.720 & $<0.001$ \\
100  & 19.719 & 19.719 & $<0.001$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
0.5 & 292.4 & 24.44 & 25 & 4146.4 & \checkmark \\
0.7 & 182.7 & 13.47 & 14 & 1979.6 & \checkmark \\
1.0 & 100.0 & 5.20  & 6  & 763.0  & \checkmark \\
1.5 & 46.8  & -0.12 & 0  & 150.4  & \checkmark \\
\bottomrule
\end{tabular}
\end{table}

$\alpha=0.5$（重尾）では $n_{\mathrm{ext}}^*=25$ 世代，$\alpha=1.5$（軽尾）では $n_{\mathrm{ext}}^*=0$ 世代（追加保持不要）：
エンタープライズ規模の全損損失（5億円）の下では，Pareto 裾の減衰が遅い重尾環境ほど
大量の追加世代保持が費用最小化に必要となる．
...
MTPD=30 日基準のパラメータ設定では $\Dinf\approx19.7$ 日（約3週間）となり，実運用と整合する．
...
\paragraph{今後の課題}
（i）定常収束速度の定量評価（有限 $\lambda$ における収束レートの明示的な上界），
（ii）位相型（phase-type）分布による復旧時間モデルの精緻化~\cite{asmussen2003}，
（iii）Hill 推定量~\cite{hill1975} と Pickands 推定量~\cite{pickands1975} による
$\alpha$ の実データからの統計的推定．
...
\textit{MIS Quarterly}, \textbf{34}(3), 567--594.
...
\bibitem{cisa2020}
CISA (2020).
\textit{Ransomware Guide}.
Cybersecurity and Infrastructure Security Agency.
\url{https://www.cisa.gov/stopransomware/ransomware-guide}.

%% --- 国内機関レポート ---
\bibitem{nisc2023}
内閣サイバーセキュリティセンター（NISC）(2023).
「重要インフラのサイバーセキュリティに係る行動計画」改定版.
...
\bibitem{jpcert2023}
JPCERT/CC (2023).
「インシデントハンドリング報告（2023 年 1 月〜12 月）」.
