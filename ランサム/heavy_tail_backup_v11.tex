...
$\lambda$ & 0.1 & 件/日 & 国内主要企業規模の推定侵入試行頻度：
  JPCERT/CC~\cite{jpcert2023} の年間インシデント報告数（約 3,000 件/年）
  を常時標的とされる組織数（約 80,000 社）で除した概算値 \\
$d_1$ & 100 & 万円/日 & $d_1=L/\mathrm{MTPD}$ の定義による（MTPD=30 日） \\
$L$ & 3000 & 万円 & ランサムウェア被害の国内平均損害額（復旧費用・機会損失合計）：
  警察庁~\cite{npa2023} および IPA~\cite{ipa2024} の中規模企業調査実績値 \\
$c_b$ & 1 & 万円$\cdot$日 & クラウドストレージ費用のオーダー推定 \\
$n$ & 3 & 世代 & 業界標準の "3-2-1 バックアップ"~\cite{cisa2020} に基づく最小世代数 \\
$\tau_R$ & 0.85 & 日 & コンテナ隔離（約 0.1 日）+ クローン（約 0.5 日）+
  再デプロイ（約 0.25 日）の合計：
  CISA~\cite{cisa2020} および NIST SP 800-184~\cite{nist80034} の
  クラウド復旧時間の中央値に基づく \\
$\eta$ & 0.9 & --- & EDR（エンドポイント検知応答）製品の
  症状検知率の実測平均~\cite{crowdstrike2023,sophos2023} \\
$\kappa_s$ & 0.5 & 日/世代 & フォレンジックスキャン所要時間の推定：
  CISA~\cite{cisa2020} のインシデント対応ガイドラインに基づく概算 \\
$\beta_{\max}$ & 0.5 & 1/日 & 高度 EDR 導入時の事前検知レート上限（感度-特異度トレードオフ）~\cite{crowdstrike2023} \\
$c_n$ & 5 & 万円/世代 & 標準クラウドストレージ（500 GB/世代）の月額費用相当 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{最適追加保持世代数（$\Delta=10$ 日，$c_n=5$ 万円）}
\label{tab:newsboy}
