...
%% ─── 区切り線 ─────────────────────────────────────────────
\draw[darkgray!30, thick] (-7.3,-3.5) -- (7.3,-3.5);

%% ═══════════ PART Ⅱ：タイムステッピングフロー ═════════════
\begin{scope}[yshift=-40mm]

\node[font=\small\bfseries\jpfont, text=navy] at (0,0)
  {\jp{タイムステッピングフロー（$t^n \to t^{n+1}$）}};

\node[statebox, below=5mm] (In)
...
\end{scope}
\end{tikzpicture}
\caption{\jp{（上）NS 方程式の各項と対応ソルバーの概要．（下）タイムステッピングフロー．
①〜⑨ は第\ref{sec:algorithm}章に対応する．}}
\label{fig:overview}
\end{figure}
...
\begin{itemize}
  \item \jp{第1項 $\dfrac{p_{i+1,j}-p_{i,j}}{h_x}$：セル界面$(i+1/2,j)$における\textbf{直接的な}2点圧力差分．
       $p_i$ 自身を参照するためチェッカーボード振動成分を「見る」ことができる．}
  \item \jp{第2項 $\dfrac{1}{2}(p^{(1)}_{x,i,j}+p^{(1)}_{x,i+1,j})$：
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt]
  ...
  \item \jp{\textbf{FVM 変密度 PPE（第\ref{sec:PPE_FVM}節）：}
        ...
        右辺に Rhie-Chow 発散\eqref{eq:RC_div}を用いることで
        Projection 法との整合性を確保する．
  ...
  \item \jp{\textbf{完全アルゴリズムフロー（第\ref{sec:algorithm}章）：}
        全9段階（Level Set 移流・格子更新・Predictor・Poisson・速度補正など）を整理した．}
