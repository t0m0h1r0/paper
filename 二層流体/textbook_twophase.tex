...
\begin{tikzpicture}[
  ...
  box/.style={draw=navy, fill=tblue, rounded corners=3pt,
              minimum width=36mm, minimum height=9mm,
              align=center, font=\small\jpfont},
  arrow/.style={-Stealth, thick, color=navy}
]
\node[box] (ns)   {\jp{Navier--Stokes 方程式}\\(One-Fluid)};
\node[box, right=18mm of ns] (ls)   {\jp{Level Set 法}\\$\partial\phi/\partial t + \bu\cdot\bnabla\phi=0$};
\node[box, below=8mm of ns]  (proj) {Projection \jp{法}\\$\bnabla\cdot\bu=0$};
\node[box, below=8mm of ls]  (ccd)  {CCD \jp{法}\\$\Ord{h^6}$ \jp{精度微分}};
\node[box, below=8mm of proj](grid) {\jp{界面適合格子}\\$\omega(\phi)$};
\node[box, below=8mm of ccd] (rhie) {Rhie--Chow \jp{補正}\\+ \jp{コロケート格子}};
\draw[arrow] (ls) -- node[above,font=\footnotesize\jpfont]{\jp{界面追跡}} (ns);
\draw[arrow] (ns) -- (proj);
\draw[arrow] (ccd) -- (proj);
\draw[arrow] (ccd) -- (ns);
\draw[arrow] (ls) -- (ccd);
\draw[arrow] (ls) -- (grid);
\draw[arrow] (grid) -- (ccd);
\draw[arrow] (rhie) -- (proj);
\end{tikzpicture}
\caption{\jp{各手法の相互関係図}}
\label{fig:overview}
\end{figure}
...
\begin{tcolorbox}[mybox, title={Rhie--Chow \jp{補正速度（セル界面）}}]
\begin{equation}
  u_{i+1/2}^{\mathrm{RC}}
  = \frac{u_i + u_{i+1}}{2}
  - \frac{\Delta t}{2\tilde\rho_{i+1/2}}
    \!\left[
      \left.\frac{\partial p}{\partial x}\right|_{i+1}^{\mathrm{CCD}}
      - \left.\frac{\partial p}{\partial x}\right|_{i}^{\mathrm{CCD}}
    \right]\frac{1}{h}
  \label{eq:rhie_chow}
\end{equation}
\end{tcolorbox}

\jp{右辺第2項は隣接セル中心の圧力勾配差であり，
チェッカーボードを引き起こす高波数成分を減衰させる役割を持つ．
これにより CCD の $\Ord{h^6}$ 精度を全変数で一貫して保ちながら，
圧力の数値不安定を防ぐことができる．}
...
\subsection{CCD \jp{による PPE の差分化}}

\jp{格子点 $(i,j)$ における各微分量を CCD で評価する：}
\begin{alignat}{2}
  (\bnabla^2 p)_{i,j}
    &= p^{(2)}_{x,i,j} + p^{(2)}_{y,i,j}
  \\[3pt]
  (\bnabla\rho)_{i,j}
    &= \bigl(\rho^{(1)}_{x,i,j},\; \rho^{(1)}_{y,i,j}\bigr)
  \\[3pt]
  (\bnabla p)_{i,j}
    &= \bigl(p^{(1)}_{x,i,j},\; p^{(1)}_{y,i,j}\bigr)
  \\[3pt]
  (\bnabla\cdot\bu^*)_{i,j}
    &= u^{*(1)}_{x,i,j} + v^{*(1)}_{y,i,j}
\end{alignat}

\jp{式\eqref{eq:PPE_expanded}に代入すると各格子点での代数方程式：}

\begin{tcolorbox}[mybox, title={\jp{離散化された圧力 Poisson 方程式（CCD 版）}}]
\begin{equation}
  \frac{p^{(2)}_{x,i,j} + p^{(2)}_{y,i,j}}{\rho_{i,j}}
  - \frac{
      \rho^{(1)}_{x,i,j}\,p^{(1)}_{x,i,j}
    + \rho^{(1)}_{y,i,j}\,p^{(1)}_{y,i,j}
  }{\rho_{i,j}^2}
  = \frac{u^{*(1)}_{x,i,j} + v^{*(1)}_{y,i,j}}{\Delta t}
  \label{eq:PPE_discrete}
\end{equation}
\end{tcolorbox}

\jp{CCD は $p^{(1)}_{x,i,j}$，$p^{(2)}_{x,i,j}$ を $\{p_{i',j}\}_{i'}$ の線形結合として表現するため，
式\eqref{eq:PPE_discrete}は $\{p_{i,j}\}$ に対する線形方程式系を構成する．
$N_x\times N_y$ 格子点の全圧力値を
$\bm{p} = [p_{1,1},\ldots,p_{N_x,N_y}]^T$ と並べると：}
\begin{equation}
  \bm{L}_{\mathrm{CCD}}\,\bm{p}^{n+1} = \bm{b}
  \label{eq:PPE_linear_system}
\end{equation}
\jp{係数行列 $\bm{L}_{\mathrm{CCD}}$ はブロック5重対角（block pentadiagonal）疎行列となる．
密度変化により対称性が一般には失われるため，
前処理付き BiCGSTAB または GMRES（ILU(0) 前処理）で求解する．}
...
\midrule
\jp{固体壁} & $\partial p/\partial n = 0$\jp{（ノイマン）} & CCD \jp{片側公式}\\
\jp{流入/流出} & $\partial p/\partial n = 0$\jp{（ノイマン）} & CCD \jp{片側公式}\\
...
\jp{Level Set 移流の数値拡散を抑えるため，
Godunov 型の上流微分と CCD を組み合わせる：}
\begin{equation}
  ...
  \label{eq:godunov_grad}
\end{equation}
\jp{ここで $D^\pm_x$ は CCD ベースの前進・後退微分値である．}
...
\medskip
\textbf{\jp{② 再初期化}}\jp{（仮想時間 $\tau$ 方向に 3〜5 反復，Godunov 勾配）}
\[
  \pd{\phi}{\tau} + \operatorname{sgn}(\phi_0)(|\bnabla\phi| - 1) = 0
...
\medskip
\textbf{\jp{⑦ 圧力 Poisson 求解}}\jp{（BiCGSTAB / GMRES，ILU(0) 前処理）}
\[
  \bm{L}_{\mathrm{CCD}}\,\bm{p}^{n+1} = \bm{b}(\bu^*,\,\tilde\rho^{n+1})
\]
...
\jp{本稿では，重力場における二次元気液混合流体の数値計算のための
完全な定式化を教科書的に解説した．主要な貢献・要点を以下にまとめる．}
...
\begin{enumerate}[leftmargin=2em, itemsep=4pt]
  ...
  \item \jp{\textbf{コロケート格子への統一（第\ref{sec:collocate}章）：}
        スタガード格子と CCD の構造的矛盾を解消するため，
        コロケート格子 + Rhie--Chow 補正を採用し，
        全変数で一貫した $\Ord{h^6}$ 精度を確保した．}
...
\begin{itemize}
  \item \jp{3次元拡張（$z$ 方向 CCD の追加と混合偏微分の増加への対応）}
  \item \jp{適応的格子細分化（AMR）との組み合わせ}
  \item \jp{MPI/OpenMP 並列化実装}
