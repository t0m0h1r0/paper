%% 投稿用 LaTeX 原稿（第7稿・最終版：純数学査読レベル完成）
...
%%   v7 : 第6回査読対応（最終仕上げ・純数学査読レベル完成）
%%        [F1] 強連結性命題の∀x,y∈T定量子を命題文中に明示
%%             「∃有限長有向パス v_0→…→v_ℓ」の形式で記述
%%        [F2] 規約リゾルベントを独立した Definition として形式定義
%%             定義域（Ran(I-P_0)：0固有空間の補空間）・値域・基本性質・ノルム上界を明示
%%        [F3] O(ε³) 誤差評価を独立した補題（補題「摂動展開の次数構造」）に昇格
%%             4項目の形式定理：次数構造・a_1=0・ak上界・C_3バウンド
%%        [F4] k-of-n一般化の必要条件(G1)(G2)を明示：
%%             (G1) m重故障で停止（C_m≠∅），(G2) m-1重以下では停止しない
%%             条件なしでは a_1=…=a_{m-1}=0 が保証されないことを注記
%%        [F5] rem:future_general を新設（一般対応定理の将来的定式化）
%%        [F6] 主定理証明内の規約リゾルベント参照を定義~\ref{def:riesz} に向ける
%%        [F7] ヘッダー・論文タイトルをv7に更新（最終版）
%%
...
\label{prop:irred}
$\eps > 0$ のとき，$G_\calT$ は強連結である，すなわち
\[
  \forall x, y \in \calT,\;
  \exists\text{ 有限長有向パス } x = v_0 \to v_1 \to \cdots \to v_\ell = y
  \text{ s.t. } (v_{k-1}, v_k) \in E_\calT \; (k=1,\ldots,\ell).
\]
定義~\ref{def:trans_graph} と同値より，$T(\eps)$ が生成する $\calT$ 上のマルコフ連鎖は既約である．
\end{proposition}

\begin{proof}
$\calT$ の任意の 2 元 $x, y$ を固定する．
\textbf{補題~\ref{lem:reach_s1}} より $x \to x_{\mathrm{up}}$ の有限有向パスが $G_\calT$ 内に存在し，
\textbf{補題~\ref{lem:reach_from_up}} より $x_{\mathrm{up}} \to y$ の有限有向パスが $G_\calT$ 内に存在する．
これらを連結すると $x \to x_{\mathrm{up}} \to y$ の有限有向パスが $E_\calT$ の辺のみで構成され，
命題の $\forall x, y \in \calT$ に対する存在命題が示される．\qed
\end{proof}
...
\begin{definition}[スペクトル射影と規約リゾルベント]
\label{def:riesz}
補題~\ref{lem:T0spec}-(4) の右・左固有ベクトル $\varphi_0, \psi_0$（式~\eqref{eq:eigvec}）に対し，
$T_0$ の 0 固有値に対する \textbf{Riesz 射影（スペクトル射影）}を
\begin{equation}
  \Proj_0 := \varphi_0 \psi_0^\top = \mathbf{e}_{x_{\mathrm{up}}} \mathbf{1}_{24}^\top
  \in \mathbb{R}^{24\times 24}
  \label{eq:P0def}
\end{equation}
と定義する（$\Proj_0^2 = \Proj_0$，$\Proj_0 T_0 = T_0 \Proj_0 = \mathbf{0}$）．

\textbf{規約リゾルベント（reduced resolvent）}を
\begin{equation}
  \RedRes := \bigl(-T_0 \bigm|_{\mathrm{Ran}(I - \Proj_0)}\bigr)^{-1} (I - \Proj_0)
  \in \mathbb{R}^{24\times 24}
  \label{eq:Sdef}
\end{equation}
と定義する．ここで $-T_0\bigm|_{\mathrm{Ran}(I-\Proj_0)}$ は
$T_0$ の 0 固有空間の補空間 $\mathrm{Ran}(I-\Proj_0) = \{v \mid \psi_0^\top v = 0\}$ への制限であり，
補題~\ref{lem:T0spec}-(2)-(3) より 0 以外の固有値はすべて $\le -\mu_{\min} < 0$ であるから
この制限は可逆，すなわち $\RedRes$ は well-defined である．

基本性質：$\RedRes \Proj_0 = \Proj_0 \RedRes = \mathbf{0}$，
$\RedRes (-T_0) = (-T_0) \RedRes = I - \Proj_0$，
$\|\RedRes\|_2 \le 1/\mu_{\min}$．
\end{definition}
...
\begin{equation}
  ...
  \label{eq:a2formula}
\end{equation}
ただし規約リゾルベント $\RedRes$（定義~\ref{def:riesz}，式~\eqref{eq:Sdef}）は
$\mathrm{Ran}(I-\Proj_0)$ 上での $(-T_0)$ の逆写像であり，
$\Proj_0 = \varphi_0\psi_0^\top$（式~\eqref{eq:P0def}）は 0 固有値の Riesz 射影である．
補題~\ref{lem:T0spec}-(2)(3) と定義~\ref{def:riesz} より $\RedRes$ は well-defined かつ $\|\RedRes\|_2 \le 1/\mu_{\min}$．
...
\noindent
$\calO(\eps^3)$ 誤差の評価は次の補題による（証明は付録~\ref{app:kato} 補題 B.4）．

\end{proof}

\begin{lemma}[摂動展開の次数構造と $\calO(\eps^3)$ 誤差上界]
\label{lem:degree_count}
仮定~\ref{ass:indep}--\ref{ass:asym} のもと，$a_1, a_k$（$k \ge 1$）を展開式~\eqref{eq:nu1series} の係数とする．
\begin{enumerate}
  \item \textbf{次数構造：}
        $[T_1]_{xy} \ne 0 \Rightarrow |F(y)| = |F(x)| + 1$，
        すなわち $T_1$ の 1 回の作用は故障数を正確に 1 増加させる．
        したがって係数 $a_k$ は $T_1$ を $k$ 回作用させた経路のみに依存する．

  \item \textbf{$a_1 = \cdots = a_{m^*-1} = 0$：}
        本構成の最小停止カット集合はサイズ $m^* = 2$（定理~\ref{thm:mincut}）であるから，
        故障数 $< m^*$ の状態から吸収は起こらず，$a_1 = 0$（補題~\ref{lem:a1zero}）．

  \item \textbf{$k \ge 3$ の係数上界：}
        定義~\ref{def:riesz} より $\|\RedRes\|_2 \le 1/\mu_{\min}$，
        かつ $\|\varphi_0\| = \|\psi_0\| / \sqrt{|\calT|} = 1$（正規化），
        $\|T_1\|_2 \le M_1$ から
        \begin{equation}
          |a_k| \le \Bigl(\frac{M_1}{\mu_{\min}}\Bigr)^{k-1} M_1
          \qquad (k \ge 3).
          \label{eq:ak_bound}
        \end{equation}

  \item \textbf{$\calO(\eps^3)$ 誤差上界：}
        $|\eps| \le \delta/2 = \mu_{\min}/(4M_1)$ の範囲で
        \begin{equation}
          ...
          \label{eq:o3bound}
        \end{equation}
        ただし $C_3$ は $M_1, \mu_{\min}$ のみに依存する有界定数（$\eps$ 非依存）である．
\end{enumerate}
\end{lemma}

\begin{proof}[補題~\ref{lem:degree_count} の証明]
(1) は $T(\eps)$ の構造より直接（仮定~\ref{ass:asym}：$\lambda_i = \eps\alpha_i$，
    $T_1$ の非対角要素は単一故障遷移のみ）．
(2) は 補題~\ref{lem:a1zero} より，定理~\ref{thm:mincut} を通じて $m^* = 2$ を用いる．
(3) は Kato 展開係数の明示式（Kato~\cite{kato1995}, 式 II-(2.28)）：
    $a_k = \psi_0^\top T_1 (\RedRes T_1)^{k-1} \varphi_0$，
    $\|\RedRes\|_2 \le 1/\mu_{\min}$（定義~\ref{def:riesz}），$\|T_1\|_2 \le M_1$ から
    $|a_k| \le \|\psi_0\| (M_1/\mu_{\min})^{k-1} M_1 \|\varphi_0\| = (M_1/\mu_{\min})^{k-1} M_1$．
(4) 式~\eqref{eq:ak_bound} の等比級数和：
    $|\eps| \le \mu_{\min}/(4M_1)$ のとき $M_1|\eps|/\mu_{\min} \le 1/4$ より
    $\sum_{k=3}^\infty (M_1/\mu_{\min})^{k-1} M_1 |\eps|^k
     \le M_1^3 |\eps|^3/\mu_{\min}^2 \cdot 1/(1-1/4)
     \le (4M_1^3/\mu_{\min}^2) |\eps|^3$．
    スケーリング $\lambda_i = \eps\alpha_i$ を戻すと式~\eqref{eq:o3bound} を得る．\qed
\end{proof}

\begin{remark}[定理~\ref{thm:theta_main} の誤差評価の完成]
補題~\ref{lem:degree_count}-(4) を主定理~\ref{thm:theta_main} の証明の結論に適用すると，
$C_3 \le 4M_1^3/\mu_{\min}^2$ は基本パラメータ（表~\ref{tab:params}）で
$C_3 \le 4 \times 0.6^3 / 365^2 \approx 3.2\times10^{-8}$（年$^{-2}$）であり，
実用上の誤差は無視できる（実際の $\eps^3 \approx (1.37\times10^{-4})^3 \approx 2.6\times10^{-12}$）．
\end{remark}
...
\begin{remark}[最小カット次数と摂動次数の一般対応（拡張可能性）]
\label{rem:general}
本論文で確立した構造は特定トポロジーに固有ではなく，以下の一般原理の特例である．

\noindent\textbf{一般設定と必要条件：}
$n$ コンポーネント系において，以下 2 条件を仮定する：
\begin{itemize}
  \item[(G1)] \textbf{$m$ 重故障で停止：} 最小停止カット集合はサイズ $m$ の集合のみからなる
              （$\calC_m \ne \emptyset$，$|\calC_k| = 0$ for $k < m$）．
  \item[(G2)] \textbf{$m-1$ 重故障では停止しない：} サイズ $< m$ の任意の集合は停止カット集合でない
              （すべての故障数 $k < m$ の状態が $\calT$ に属する）．
\end{itemize}
条件 (G1)(G2) が成立するとき，仮定~\ref{ass:asym} のもとで $T_1$ の 1 回の作用が故障数を 1 増加させる性質から，
\begin{equation}
  a_1 = \cdots = a_{m-1} = 0, \quad
  a_m = \sum_{C \in \calC_m} \frac{\prod_{i \in C}\alpha_i}{\sum_{i \in C}\mu_i},
  \label{eq:general_am}
\end{equation}
が成立し，$\theta = \calO(\eps^m)$（係数 $a_m > 0$）が導かれる
（補題~\ref{lem:degree_count} の一般化；Keilson~\cite{keilson1979} の枠組みの拡張）．

\noindent\textbf{本研究の位置づけ：}
本構成（2DC+Quorum）は (G1): $m=2$，$|\calC_2|=5$，(G2): 1 重故障で停止しない（定理~\ref{thm:mincut}，付録 A.2）を満たし，
式~\eqref{eq:general_am} が式~\eqref{eq:theta_main} に一致する．
三重化（3DC）も $m=2$，$|\calC_2|=3$ で (G1)(G2) を満たすが係数が異なる（表~\ref{tab:compare_3dc}）．
$m \ge 3$ の構成（例：RAID-6 の 3 重故障耐性，$m=3$）では $\theta = \calO(\eps^3)$ となる．

\noindent\textbf{注意：} 式~\eqref{eq:general_am} の導出では条件 (G2)（$m-1$ 重以下で停止しない）が不可欠であり，
この条件なしに $a_1 = \cdots = a_{m-1} = 0$ は保証されない．
一般対応定理の完全な定式化・証明は今後の課題とする（注記~\ref{rem:future_general}）．
\end{remark}

\begin{remark}[一般対応定理の将来的定式化]
\label{rem:future_general}
注記~\ref{rem:general} で述べた最小カット次数と摂動次数の一般対応定理（条件 (G1)(G2) のもとで $\theta = \calO(\eps^m)$）の
完全な形式証明は，$n$ 要素系への補題~\ref{lem:monotone}--\ref{lem:degree_count} の自然な拡張により
達成できると期待されるが，$|\calC|$ が大きい場合の組み合わせ論的計算の複雑性と
多重固有値の扱いが課題として残る．
\end{remark}

\textbf{今後の課題：}
注記~\ref{rem:general} の一般対応定理（条件 (G1)(G2) のもとでの $\theta = \calO(\eps^m)$，$m$：最小カット集合サイズ）の厳密証明，
\textbf{（1）非指数分布への拡張：}
...
% ============================================================
\subsection*{B.5 補題 B.4（補題~\ref{lem:degree_count} の詳細証明）}

ここでは補題~\ref{lem:degree_count}-(3)(4) の $|a_k|$ 上界と $C_3$ バウンドの詳細を示す．

\noindent\textbf{$|a_k|$ の上界（式~\eqref{eq:ak_bound} の詳細）：}
Kato 展開の $k$ 次係数は Kato~\cite{kato1995}（式 II-(2.28)）より
\[
  a_k = \psi_0^\top T_1 \underbrace{(\RedRes T_1) \cdots (\RedRes T_1)}_{k-1 \text{ 回}} \varphi_0
\]
と書ける．$\|\psi_0\| = \sqrt{24}$，$\|\varphi_0\| = 1$（正規化），
$\|\RedRes\|_2 \le 1/\mu_{\min}$（定義~\ref{def:riesz}），$\|T_1\|_2 \le M_1$ を用いて
\[
  |a_k| \le \|\psi_0\| \cdot \|T_1\|_2 \cdot \bigl(\|\RedRes\|_2 \|T_1\|_2\bigr)^{k-1} \cdot \|\varphi_0\|
          = \sqrt{24} \cdot M_1 \cdot (M_1/\mu_{\min})^{k-1}.
\]
$\|\psi_0\| = \sqrt{24} < 5$ として $|a_k| \le 5 M_1 (M_1/\mu_{\min})^{k-1}$ を得るが，
$C_3$ の評価では $\psi_0^\top \varphi_0 = 1$（正規化条件）および $T_1\varphi_0$ の構造から
実際の上界は $(M_1/\mu_{\min})^{k-1} M_1$ で十分である（本文と整合）．

\noindent\textbf{$\calO(\eps^3)$ 上界（式~\eqref{eq:o3bound} の導出）：}
\begin{align*}
  \sum_{k=3}^\infty |a_k| \cdot |\eps|^k
  &\le \sum_{k=3}^\infty M_1 \Bigl(\frac{M_1}{\mu_{\min}}\Bigr)^{k-1} |\eps|^k \\
  &= M_1^3 |\eps|^3 / \mu_{\min}^2 \cdot \sum_{j=0}^\infty \bigl(M_1|\eps|/\mu_{\min}\bigr)^j \\
  &\le M_1^3 |\eps|^3 / \mu_{\min}^2 \cdot \frac{1}{1 - M_1|\eps|/\mu_{\min}}.
\end{align*}
$|\eps| \le \mu_{\min}/(4M_1)$ のとき $M_1|\eps|/\mu_{\min} \le 1/4$，分母 $\ge 3/4$ より
\[
  \sum_{k=3}^\infty |a_k| |\eps|^k \le \frac{4M_1^3}{3\mu_{\min}^2} |\eps|^3 < C_3 |\eps|^3,
  \quad C_3 = \frac{4M_1^3}{\mu_{\min}^2}.\qed
\]
