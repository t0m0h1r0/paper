...
すなわち $T(\eps)$ が生成する $\calT$ 上のマルコフ連鎖は既約である：
任意の $x, y \in \calT$ に対して，$G_\calT$ 内に $x$ から $y$ への有向パスが存在する．
\end{proposition}

\begin{proof}
$G_\calT$ の強連結性，すなわち任意の $x, y \in \calT$ に対して $G_\calT$ 内で
$x \to y$ の有向パスが存在することを構成的に示す．
任意の $x, y \in \calT$ を固定し，以下 2 段階で具体的なパスを構成する．
...
一般行列では $|\partial\nu_k/\partial\eps| \le c_n \|T_1\|_2 = c_n M_1$（$c_n \le 24$ は次元に依存する定数）．
よって $C = c_n M_1$ と取れ，$\eps < \mu_{\min}/(2C) =: \eps_*$ のとき $\Re(\nu_k) \le -\mu_{\min}/2$．

この $\eps_*$ は基本パラメータ（表~\ref{tab:params}）で $\eps_* \ge \mu_{\min}/(2\cdot 24 M_1) \approx 365/(48\times0.6) \approx 12.7$（年）であり，
